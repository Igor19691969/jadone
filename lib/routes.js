...
*/
   ...
   // app.get('/api/userslistold',api.userOldList);
    ...
    app.get('/*',middleware.redirect,middleware.versionBrowser, middleware.setUserCookie, function(req, res,next){
        var me = req.user || undefined;
        var config = require("../data/config.json");
        async.parallel([
            function(callback){
                Category.find({section:"535f69c5377721f019fef375"})
                    .sort('index')
                    .select('name index desc img filters')
            ...
            function(err, results){
                if (err)return next(err);
                    ...
                    return res.render('index',{config:config,categories:results[0],filters:results[1],user:me,stats:results[2]});
                }
            });
    })
};