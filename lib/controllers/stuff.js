...
exports.list= function(req, res) {
    ...
    if (req.query && req.query.searchStr){
                ...
                Category.find({section:req.params.category},function (err, categories) {
                    ...
                    if (filterTags){
                        ...
                        if (filterTags.length==1){
                            var query={tags: filterTags[0]};
                        } else {
                            var arr=[];
                            for (var i= 0,l=filterTags.length;i<l;i++){
                                arr[arr.length]={tags:filterTags[i]}
                            }
                            console.log(arr);
                            var query={$and: arr};
                        }
                        ...
                        options.criteria={ $and: [{ category:  {$in: categoryArr}},query]};
                    }
                    console.log(options);
                    Stuff.list(options, function(err, stuffs) {
                        if (err)  return res.json(err);
        ...
        if (filterTags){
            ...
            options.criteria={ $and: [{ category:req.params.category  },query]};
        }
