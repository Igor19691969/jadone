        ...
        this.pay = function(req,res,next){
            ...
            Order.findOne({num:req.params.num}, function (err,doc) {
                ...
                var mailOptions = {
                    ...
                    subject: "оплата ✔", // Subject line
                    to: 'jadoneopt@gmail.com', // list of receivers
                    text:"Cообщение ОБ ОПЛАТЕ ЧЕРЕЗ СИСТЕМУ LIQPAY."+'\n'+
                        "Произведена оплата по ордеру номер - "+req.params.num+'\n'+
                        "Поступление платежа уточняйте у ответственного лица!"+'\n'+
                        "Внимание. Данное сообщение является только информационным и не является основанием для отправки заказа!!!"
                }
        ...
        if (req.query.invoice){
            ...
            if (req.body._id){
                Order.update({_id: req.body._id},
                    {$set:{invoice:req.query.invoice}}, function(e, result){
                        if (e) return next(e)
                        Order.findOne({_id: req.body._id})
                            .populate('user','email')
                            .exec(function(error, order){
                            if (error) return next(error)
                                // если пользователь зпрегистрирован по номеру телефона то письмо не отправляем
                                var arr =  order.user.email.split('@')
                                if(arr[1] && arr[1]=='no.no'){
                                    //console.log('sss');
                                    return res.send((result === 1) ? {msg:'success'} : {msg:'error'})
                                }
                                //********************************************************************************
                            var smtpTransport = nodemailer.createTransport("SMTP",{
                                service: "Mailgun",
                            ...
                            var message=
                                '<html>'+
                                    ...
                                    '<h3>Здравствуйте!</h3>'+
                                    '<p>Вы можете оплатить заказ из  <a href="jadone.biz/ru/customorder">ЛИЧНОГО КАБИНЕТА</a> либо по следующим реквизитам:' +
                                    '</br>'+

                                "1.Карта СберБанка России:\t"+
                                "   Маслов Олег Николаевич\n"+
                                "   5469 0700 1157 5558\n"+
                                'В назначении платежа укажите – перевод личных средств'+
                                ' или денежным переводом'+
                                'г. Харьков Маслов Олег Николаевич'+
                                '   +380631310931</br>'+


                                    '<p>Номер заказа: '+order.num+'</p>'+
                                    'от : '+moment(order.date).format('LLL')+
                                    '<p>количество - ' +getTotalQuantity(order)+' единиц'+
                                    '</p>'+
                                    '<p>Сумма c доставкой - ' +(shipCost+order.kurs*orderSum(order,order.quantity,true)).toFixed(2)+' '+order.currency+
                                    '</p>';
                                /*console.log((shipCost+order.kurs*orderSum(order,order.quantity,true)).toFixed(2))
                                return;
*/


                                    mailOptions.html=message;
                            mailOptions.to=order.user.email;
                            smtpTransport.sendMail(mailOptions, function(error, response){
                                if(error){
                                    next(error);
                                }else{
                ...
                if (upsertData.num){ // update
                    ...
                    Order.findOne({user:upsertData.user, $or:[{status:1},{status:2}]}).exec(function(err,order){
                        ...
                        if (!order) { // новый ордер и нет открытого ордера
                            ...
                            // дата дозаказа не более пяти дней с момента заказа
                            /*console.log(order.date);
                            console.log(upsertData.date);
                            console.log(Date.parse(upsertData.date)-Date.parse(order.date))*/
                            if (Date.parse(upsertData.date)-Date.parse(order.date)>(24*60*60*1000)){
                                // todo или выставлен счет
                                // разница более пяти дней
                                // новый заказ
                                setNewOrder();
                            } else {
                                // дозаказ
                    ...
                    User.findById(order.user).exec(function (err,user){
                        ...
                        var messageBegin2=
                            '<html>'+
                                ...
                                'Вы сможете оплатить заказ из  <a href="jadone.biz/ru/customorder">ЛИЧНОГО КАБИНЕТА</a> после сбора заказа и выставления счета.' +
                                ' Предварительную дату сбора заказа вы можете посмотреть в личном кабинете на сайте.'+
                                '<p>Номер заказа: '+upsertData.num+'</p>'+
                                'Дата : '+moment(dateForTable).format('LLL')+'<br>';
